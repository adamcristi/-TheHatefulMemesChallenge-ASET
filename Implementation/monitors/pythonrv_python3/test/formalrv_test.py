...
from pythonrv_python3.formalrv import formal_spec, make_assert, make_next, make_if

class TestFormalDetails(unittest.TestCase):
    def test_assert_cases(self):
        self.assertEqual(len(make_assert()), 0)
...
class TestFormalRV(unittest.TestCase):
    ...
    def test_simple_if_else(self):
        for i in range(2):
            ...
            with self.assertRaises(AssertionError) as e:
                a.m(i)
            self.assertEqual(e.exception.message, "heisenberg%d" % i)
