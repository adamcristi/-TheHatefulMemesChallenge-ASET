...
class TestOnFunctions(unittest.TestCase):
    ...
    def test_existing_func_attribute(self):
        ...
        self.assertEqual(func_attributes.test_attrib, 'test')
    ...
    def test_explicit_call(self):
        ...
        res = func_zorro(None)
        self.assertEqual(res, "zorro")

        with self.assertRaises(ValueError) as e:
            func_zorro('p')
        self.assertEqual(e.exception.message, 'p')

        with self.assertRaises(ValueError) as e:
            func_zorro('m')
        self.assertEqual(e.exception.message, 'm')

        with self.assertRaises(ValueError) as e:
            func_zorro('n')
        self.assertEqual(e.exception.message, 'n')
    ...
    def test_calling_cond_alone(self):
        ...
        self.assertEqual(q(None, 3), 7)

        @dbc.after(Aloha.m)
        def z(self, x):
            return 17

        self.assertEqual(z(None, None), 17)
...
class TestOnClassFunctions(unittest.TestCase):
    ...
    def test_class_method(self):
        ...
        self.assertEqual(class_res, 8)
        self.assertEqual(Aloha.val, 'qab')

        Aloha.val += '_'
        obj = Aloha()
        obj_res = obj.cm(2)
        self.assertEqual(obj_res, 3)
        self.assertEqual(Aloha.val, 'qab_ab')
    ...
    def test_object(self):
        ...
        self.assertEqual(a.m(7), 7)

        with self.assertRaises(ValueError) as e:
            b.m(8)
        self.assertEqual(e.exception.message, "buffy b")
        ...
        with self.assertRaises(ValueError) as e:
            a.m('a')
        self.assertEqual(e.exception.message, "spike a")
        self.assertEqual(a.m(7), 7)
        ...
        with self.assertRaises(ValueError) as e:
            b.m(8)
        self.assertEqual(e.exception.message, "angel b2")
