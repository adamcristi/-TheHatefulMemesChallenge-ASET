...
class TestConfiguration(unittest.TestCase):
    def test_configuration(self):
        ...
        self.assertEqual(rv.get_configuration()['error_handler'], rv.DEFAULT_ERROR_HANDLER)
        self.assertEqual(rv.get_configuration()['enable_copy_args'], True)

        rv.configure(error_handler=1234, enable_copy_args=False)
        self.assertEqual(rv.get_configuration()['error_handler'], 1234)
        self.assertEqual(rv.get_configuration()['enable_copy_args'], False)
    ...
    def test_enable_copy_args(self):
        ...
        def spec(event):
            ...
            self.assertEqual(event.fn.m.outputs[0].x, 123)
    ...
    def test_cannot_copy_cstringio(self):
        ...
        with self.assertRaises(TypeError) as e:
            copy.deepcopy(cs)
        self.assertEqual('object.__new__(cStringIO.StringO) is not safe, use cStringIO.StringO.__new__()', e.exception.message)
...
class TestRaiseExceptionErrorHandler(unittest.TestCase):
    def test_defaults(self):
        ...
        self.assertEqual(e.exception.message, 'buffy4')

        with self.assertRaises(AssertionError) as e:
            h.handle(rv.CRITICAL, [AssertionError('buffy5')])
        self.assertEqual(e.exception.message, 'buffy5')
        ...
        for level_i, level in enumerate(error_levels):
            ...
            for i, t in enumerate(error_levels):
                ...
                if i < level_i:
                    ...
                    with self.assertRaises(AssertionError) as e:
                        a.m()
                    self.assertEqual(e.exception.message, 'of time')
