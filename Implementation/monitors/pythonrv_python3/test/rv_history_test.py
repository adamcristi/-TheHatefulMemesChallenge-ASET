...
class TestHistory(unittest.TestCase):
    ...
    def test_event_simple(self):
        ...
        @rv.monitor(m=M.m)
        def spec(event):
            self.assertEqual(event.history[0].prev, None)
        ...
        for i in range(rv.DEFAULT_MAX_HISTORY_SIZE):
            with self.assertRaises(ValueError) as e:
                a.m()
            self.assertEqual(e.exception.message, "call %d" % (i+1))
    ...
    def test_event_inputs_and_outputs(self):
        ...
        for i in range(1,10):
            with self.assertRaises(ValueError) as e:
                a.m(dict(x=i))
            self.assertEqual(e.exception.message, "prev vals %d" % ((i-1)*2 + i))
    ...
    def test_monitor_simple(self):
        ...
        for i in range(1,10):
            with self.assertRaises(ValueError) as e:
                a.m()
            self.assertEqual(e.exception.message, "call %d" % i)
        ...
        for i in range(35):
            ...
            for j in range(1, 80):
                with self.assertRaises(ValueError) as e:
                    a.m()
                if (j <= history_size):
                    self.assertEqual(e.exception.message, "%d %d" % (j, j))
                else:
                    self.assertEqual(e.exception.message, "%d %d" % (history_size, history_size))
    ...
    def test_infinite_history_size(self):
        ...
        for i in range(200):
            with self.assertRaises(ValueError) as e:
                a.m()
            self.assertEqual(e.exception.message, i+1)
    ...
    def test_negative_history_size(self):
        ...
        with self.assertRaises(ValueError) as e:
            ...
            def spec(event):
                raise ValueError(len(event.history))
        self.assertEqual(e.exception.message, "Negative max history sizes (-3) are not allowed")
    ...
    def test_many_monitors(self):
        ...
        @rv.spec_save_automatically()
        def spec(event):
            self.assertEqual((len(event.history)-1)/3, event.called_function.inputs[1])
            self.assertEqual(len(event.called_function.history), event.called_function.inputs[1]+1)
