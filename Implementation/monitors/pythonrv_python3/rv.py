...
def monitor(**monitorees):
    def decorator(spec):
        ...
        for name, func in list(monitorees.items()):
            obj = None
...
def _cleanup_spec(spec, spec_info):
    ...
    if len(spec_info.oneshots) > 0:
        ...
        return
    if len([oneshot for monitor in list(spec_info.monitors.values()) for oneshot in monitor.oneshots]) > 0:
        # some monitor has remaining oneshots
        return

    for monitor in list(spec_info.monitors.values()):
...
class EventDataFunctions(object):
    def __init__(self, spec_info, data):
        self._functions = []
        for name, monitor in list(spec_info.monitors.items()):
...
class LoggingErrorHandler(object):
    def __init__(self):
        self.logger = logging.getLogger('pythonrv_python3')
